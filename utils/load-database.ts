...
export const loadDatabase = async () => {
  ...
  const allUsersRows = (await db.getAllAsync("SELECT * FROM Users")) as any;
  for (const row of allUsersRows) {
    console.log(row.id_user, row.name, row.password, row.creation_date);
  }

  const allStudentsRows = (await db.getAllAsync(
    "SELECT * FROM Students"
  )) as any;
  for (const row of allStudentsRows) {
    console.log(row.id_user, row.generation, row.grade, row.major, row.note);
  }

  const allTeachersRows = (await db.getAllAsync(
    "SELECT * FROM Teachers"
  )) as any;
  for (const row of allTeachersRows) {
    console.log(row.id_teacher, row.id_user, row.field);
  }

  const allClassesRows = (await db.getAllAsync("SELECT * FROM Classes")) as any;
  for (const row of allClassesRows) {
    console.log(row.id_class, row.id_teacher, row.name, row.code, row.cover);
  }

  const allAssistsRows = (await db.getAllAsync("SELECT * FROM Assists")) as any;
  for (const row of allAssistsRows) {
    console.log(
      row.id_attendance,
      row.id_student,
      row.id_class,
  ...
  const allGradesRows = (await db.getAllAsync("SELECT * FROM Grades")) as any;
  for (const row of allGradesRows) {
